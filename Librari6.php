...
class Library
{
    ...
    {
        $data = $this->db->prepare('INSERT INTO bahasa (namaMateri,kategori,jenisPembelajaran) VALUES (?, ?, ?)');
    ...
    public function get_by_id($id_materi){
        $query = $this->db->prepare("SELECT * FROM bahasa where id_materi=?");
    ...
    public function update($id_materi,$namaMateri,$kategori,$jenisPembelajaran){
        $query = $this->db->prepare('UPDATE bahasa set namaMateri=?,kategori=?,jenisPembelajaran=? where id_materi=?');
    ...
    {
        $query = $this->db->prepare("DELETE FROM bahasa where id_materi=?");
