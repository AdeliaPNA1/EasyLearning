...
class Library
{
    ...
    {
        $data = $this->db->prepare('INSERT INTO sastra (namaMateri,kategori,jenisPembelajaran) VALUES (?, ?, ?)');
    ...
    {
        $query = $this->db->prepare("SELECT * FROM sastra");
        $query->execute();
        $data = $query->fetchAll();
        return $data;
    }
    ...
    public function get_by_id($id_materi){
        $query = $this->db->prepare("SELECT * FROM sastra where id_materi=?");
    ...
    public function update($id_materi,$namaMateri,$kategori,$jenisPembelajaran){
        $query = $this->db->prepare('UPDATE sastra set namaMateri=?,kategori=?,jenisPembelajaran=? where id_materi=?');
    ...
    {
        $query = $this->db->prepare("DELETE FROM sastra where id_materi=?");
